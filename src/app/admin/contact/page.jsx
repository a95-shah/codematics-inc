    ...
    <div className="space-y-12 animate-fade-in font-body">
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/5 pb-10 gap-6">
        <div className="space-y-4">
          ...
          <p className="text-gray-400 font-medium max-w-lg leading-relaxed border-l-2 border-white/10 pl-6">Respond to customer inquiries, support requests, and potential leads.</p>
        </div>
      </div>
                ...
                <tr key={msg._id} className={`hover:bg-white/[0.02] transition-colors group ${!msg.isRead ? 'bg-[#c92228]/5 border-l-2 border-l-[#c92228]' : ''}`}>
                  <td className="px-8 py-6 whitespace-nowrap">
                    <div className="flex items-center gap-4">
                      <div className={`h-10 w-10 flex-shrink-0 rounded-full flex items-center justify-center font-bold text-lg overflow-hidden transition-colors ${msg.isRead ? 'bg-white/5 text-gray-500 border border-white/5' : 'bg-[#c92228] text-white shadow-[0_0_15px_rgba(201,34,40,0.4)]'}`}>
                        {msg.name.charAt(0)}
                      </div>
                      <div>
                        <div className="text-sm font-bold text-white font-heading group-hover:text-[#c92228] transition-colors tracking-tight">{msg.name}</div>
                  ...
                  <td className="px-8 py-6">
                    <div className="max-w-xs md:max-w-md lg:max-w-xl">
                      <div className="text-sm font-bold text-white truncate mb-2">{msg.subject || 'Website Inquiry'}</div>
                    ...
                    <button 
                      ...
                      className={`px-4 py-1.5 rounded-full text-[9px] font-bold tracking-widest transition-all ${
                        msg.isRead 
                          ...
                          : 'bg-[#c92228] text-white shadow-lg border border-[#c92228] animate-pulse cursor-pointer'
                      }`}
                    >
                      {msg.isRead ? 'READ' : 'NEW'}
                    </button>
                  ...
                  <td className="px-8 py-6 whitespace-nowrap text-right text-sm font-medium space-x-3">
                    <button 
                      ...
                      className="text-gray-400 hover:text-white p-3 bg-white/5 hover:bg-[#c92228] rounded-xl transition-all active:scale-90 inline-flex items-center justify-center"
                    >
                      {msg.isRead ? <HiMail className="h-4 w-4" /> : <HiMailOpen className="h-4 w-4" />}
                    </button>
                    <button 
                      onClick={() => handleDelete(msg._id)} 
                      className="text-gray-400 hover:text-white p-3 bg-white/5 hover:bg-red-900 rounded-xl transition-all active:scale-90 inline-flex items-center justify-center"
                    >
