    ...
    <div className="space-y-12 animate-fade-in font-body">
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-white/5 pb-10 gap-6">
        <div className="space-y-4">
          ...
          <p className="text-gray-400 font-medium max-w-lg leading-relaxed border-l-2 border-white/10 pl-6">Manage your company's personnel and leadership profiles.</p>
        </div>
        {!isEditing && (
          <button 
              ...
              <tr key={member._id} className="hover:bg-white/[0.02] transition-colors group">
                <td className="px-8 py-6 whitespace-nowrap">
                   <div className="flex items-center gap-4">
                      <div className="h-10 w-10 flex-shrink-0 rounded-full bg-white/5 border border-white/5 overflow-hidden group-hover:border-[#c92228]/50 transition-colors flex items-center justify-center">
                      ...
                      <div>
                         <div className="font-bold text-white text-sm font-heading group-hover:text-[#c92228] transition-colors tracking-tight">{member.name}</div>
                ...
                <td className="px-8 py-6 whitespace-nowrap text-right text-sm font-medium space-x-3">
                  <button onClick={() => handleEdit(member)} className="text-gray-400 hover:text-white p-3 bg-white/5 hover:bg-[#c92228] rounded-xl transition-all active:scale-90"><HiPencil className="h-4 w-4" /></button>
                  <button onClick={() => handleDelete(member._id)} className="text-gray-400 hover:text-white p-3 bg-white/5 hover:bg-red-900 rounded-xl transition-all active:scale-90"><HiTrash className="h-4 w-4" /></button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="bg-[#232527] p-12 lg:p-16 rounded-[3rem] shadow-3xl border border-white/5 mt-20 relative overflow-hidden group">
        ...
        <h2 className="text-3xl font-black text-white mb-12 font-heading tracking-tighter">{isEditing ? 'Update Member Profile' : 'Register New Member'}</h2>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="space-y-8">
            <div className="space-y-3">
              ...
              <input type="text" className="w-full px-8 py-5 rounded-2xl bg-[#1B1D1F] border-2 border-white/5 focus:border-[#c92228] text-white transition-all font-bold tracking-tight outline-none" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
            </div>
            <div className="space-y-3">
              <label className="block text-[10px] font-bold text-gray-500 tracking-widest ml-4">Professional Role</label>
              <input type="text" className="w-full px-8 py-5 rounded-2xl bg-[#1B1D1F] border-2 border-white/5 focus:border-[#c92228] text-white transition-all font-bold tracking-tight outline-none" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} placeholder="e.g. Lead Developer" />
            </div>
            <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-8">
              <div className="space-y-3">
                ...
                <input type="number" className="w-full px-8 py-5 rounded-2xl bg-[#1B1D1F] border-2 border-white/5 focus:border-[#c92228] text-white transition-all font-bold outline-none" value={form.order} onChange={(e) => setForm({ ...form, order: parseInt(e.target.value) || 0 })} />
              </div>
              <div className="flex items-center p-5 bg-[#1B1D1F] rounded-2xl border border-white/5 group/check cursor-pointer mt-7" onClick={() => setForm({ ...form, isActive: !form.isActive })}>
                <div className={`h-5 w-5 rounded-md border-2 flex items-center justify-center transition-all ${form.isActive ? 'bg-[#c92228] border-[#c92228]' : 'border-white/10'}`}>
                   {form.isActive && <HiCheck className="text-white h-3 w-3" />}
                </div>
          ...
          <div className="space-y-8">
            <div className="space-y-3">
              ...
              <input type="text" className="w-full px-8 py-5 rounded-2xl bg-[#1B1D1F] border-2 border-white/5 focus:border-[#c92228] text-[#c92228] transition-all font-mono text-xs outline-none" value={form.linkedin} onChange={(e) => setForm({ ...form, linkedin: e.target.value })} placeholder="https://linkedin.com/in/..." />
            </div>
            <div className="space-y-3">
              <label className="block text-[10px] font-bold text-gray-500 tracking-widest ml-4">Professional Biography</label>
              <textarea className="w-full px-8 py-5 rounded-2xl bg-[#1B1D1F] border-2 border-white/5 focus:border-[#c92228] text-white transition-all min-h-[220px] font-medium leading-relaxed outline-none" value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} />
            </div>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end space-x-6 pt-10 border-t border-white/5">
            ...
            <button type="submit" className="px-12 py-5 bg-[#c92228] text-white rounded-2xl shadow-3xl hover:bg-[#a01b20] transition-all transform active:scale-95 font-bold text-xs shadow-red-900/40">Save Profile</button>
          </div>
        </form>
      </div>
    </div>
