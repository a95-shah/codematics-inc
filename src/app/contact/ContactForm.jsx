      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="pt-4">
          <button 
            type="submit" 
            disabled={loading}
            className="w-full py-5 bg-gray-900 text-white rounded-2xl text-xs font-black uppercase tracking-[0.5em] shadow-2xl hover:bg-blue-600 transition-all transform active:scale-95 disabled:bg-gray-400 flex items-center justify-center group/btn"
          >
            {loading ? 'Transmitting...' : 'Initiate Transmission'}
            <span className="ml-4 h-6 w-6 rounded-full bg-white/10 group-hover/btn:bg-white/20 flex items-center justify-center transition-all group-hover/btn:translate-x-2">→</span>
          </button>
        </div>
      </form>
    </div>
