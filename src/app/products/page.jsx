      ...
      <section className="pb-32 container mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {products.map((product) => (
            <Link 
              ...
              className="group relative bg-[#232527] rounded-[2.5rem] overflow-hidden shadow-2xl transition-all duration-500 border border-white/5 hover:border-red-600/20 flex flex-col h-full"
            >
               {/* Product Decoration */}
               <div 
               ...
               <div className="p-10 relative z-10 flex flex-col h-full">
                  <div className="mb-8 flex justify-between items-start">
                    <div className="flex flex-wrap gap-2">
                       {product.platforms?.map((p, i) => (
                         <span key={i} className="px-3 py-1 bg-white/5 text-[9-px] font-bold text-gray-500 rounded-full border border-white/5 tracking-wider uppercase">{p}</span>
                       ))}
                    </div>
                  ...
                  <h2 className="text-2xl font-black text-white mb-4 font-heading tracking-tight group-hover:text-[#c92228] transition-colors">{product.title}</h2>
                  <p className="text-gray-400 text-[15px] font-medium leading-relaxed mb-10 line-clamp-3">{product.description}</p>
        ...
        {products.length === 0 && (
          <div className="text-center py-40 bg-white/5 rounded-[3rem] border border-dashed border-white/10">
             <p className="text-gray-600 font-bold tracking-[0.3em]">INNOVATION PIPELINE CURRENTLY EMPTY</p>
          </div>
        )}
      </section>
    </div>
