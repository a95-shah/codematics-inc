# OPUS IMPLEMENTATION PROMPT
# Copy everything below this line and paste into Claude Opus 4.6
​
---
​
You are an expert Next.js 14 fullstack developer. Your job is to IMPLEMENT —
not explain, not describe, not summarize. Write every single file completely,
with full working code. No placeholders. No "// add your logic here".
No truncation. Every file must be production-ready and copy-paste ready.
​
---
​
CONTEXT:
I have an existing Next.js 14 company website using App Router. These frontend
pages are already built (do NOT recreate them, just integrate):
​
- Home, About Us, Services (+ detail), Our Team (+ detail),
  Products (+ detail), News (+ detail), Contact, Remote Resources (+ detail)
​
My existing static data files are:
- /data/servicesData.ts
- /data/productsData.ts
- /data/newsData.ts
- /data/teamData.ts
​
I am providing the EXACT content of these files below. You must use this real
data to create the MongoDB seed file. Do not invent or change any data.
​
---
​
EXISTING STATIC DATA:
​
### servicesData.ts (current data to migrate):
```
services = [
  {
...
```
​
### productsData.ts (current data to migrate):
```
products = [
...
```
​
### newsData.ts (current data to migrate):
```
newsItems = [
...
```
​
### teamData.ts (current data to migrate):
```
teamMembers = [
...
Print the complete folder/file tree of everything you are about to create.
​
---
​
STEP 2 — ENV FILE
Write the complete .env.local file:
- MONGODB_URI
- NEXTAUTH_SECRET
- NEXTAUTH_URL
- ADMIN_EMAIL
...
Write complete file: /lib/db.js
- Singleton Mongoose connection with global caching
- Works in dev and production without multiple connection warnings
​
---
​
STEP 4 — MONGOOSE MODELS
Write each model as a complete separate file under /lib/models/
​
Service.js:
  slug (String, unique, required), iconName (String), title (String, required),
  description (String), longDescription (String), details ([String]),
  features ([String]), technologies ([String]),
  isActive (Boolean, default true), timestamps: true
​
Product.js:
  slug (String, unique, required), title (String, required),
  description (String), color (String), longDescription (String),
  features ([String]), platforms ([String]),
  image (String), isActive (Boolean, default true), timestamps: true
​
NewsPost.js:
  slug (String, unique, required), title (String, required),
  description (String), date (String), category (String),
  content (String), coverImage (String), author (String),
  isActive (Boolean, default true), timestamps: true
​
TeamMember.js:
  name (String, required), role (String), bio (String),
  image (String), linkedin (String), order (Number, default 0),
  isActive (Boolean, default true), timestamps: true
​
ContactMessage.js:
  name (String, required), email (String, required), phone (String),
  subject (String), message (String, required),
  isRead (Boolean, default false), createdAt (Date, default Date.now)
​
RemoteResource.js:
  title (String, required), description (String), skills ([String]),
  experience (String), image (String),
  isActive (Boolean, default true), timestamps: true
​
Page.js:
  slug (String, unique, required), title (String), heroTitle (String),
  heroSubtitle (String), sections ([{ heading: String, body: String }]),
  updatedAt (Date, default Date.now)
​
---
​
STEP 5 — DATABASE SEED FILE
Write complete file: /scripts/seed.js
​
This file must:
- Connect to MongoDB using the MONGODB_URI from .env.local
- Use dotenv to load env variables: require('dotenv').config({ path: '.env.local' })
- Clear existing data in all collections before inserting
- Insert ALL of the exact data provided above in the EXISTING STATIC DATA section
- For teamMembers: map "role" field to "role" in TeamMember model, set order: 0,1,2... based on array index
- For newsItems: map "date" field correctly, set author: "Codematics Team" for all
- For services: include ALL fields (iconName, details, features, technologies)
- For products: include ALL fields (color, longDescription, features, platforms)
- Print success/error message and exit process when done
- Run with: node scripts/seed.js
​
---
​
STEP 6 — API ROUTES
Write every file completely. Each route must:
- Handle errors with try/catch
- Return proper HTTP status codes (200, 201, 400, 401, 404, 500)
- For POST, PUT, DELETE: check session with getServerSession(authOptions) — return 401 if no session
- GET routes: support ?active=true query param to filter isActive records
​
Write ALL these files IN FULL:
​
/app/api/services/route.js          → GET all, POST new
/app/api/services/[id]/route.js     → GET one by id OR slug, PUT, DELETE
/app/api/products/route.js          → GET all, POST new
/app/api/products/[id]/route.js     → GET one by id OR slug, PUT, DELETE
/app/api/news/route.js              → GET all, POST new
/app/api/news/[id]/route.js         → GET one by id OR slug, PUT, DELETE
/app/api/team/route.js              → GET all (sorted by order), POST new
/app/api/team/[id]/route.js         → GET one, PUT, DELETE
/app/api/contact/route.js           → GET all (admin), POST (public — no auth)
/app/api/contact/[id]/route.js      → GET one, PUT (mark read/unread), DELETE
/app/api/remote-resources/route.js  → GET all, POST new
/app/api/remote-resources/[id]/route.js → GET one, PUT, DELETE
/app/api/pages/route.js             → GET all, POST new
/app/api/pages/[slug]/route.js      → GET by slug, PUT update
​
---
​
STEP 7 — AUTH SETUP
...
​
/lib/authOptions.js:
- Export authOptions with CredentialsProvider
- Validate against ADMIN_EMAIL + ADMIN_PASSWORD from process.env
- Return { id: "1", name: "Admin", email } on success, null on failure
...
Write complete file: /app/admin/login/page.jsx
- "use client"
- Clean centered card design with Tailwind
- Email + password fields with useState
- Uses signIn('credentials', { redirect: false, ... })
- Shows error message on failure
- On success: router.push('/admin/dashboard')
- Company name "Codematics Admin" at top
​
---
​
STEP 9 — ADMIN LAYOUT
Write complete file: /app/admin/layout.jsx
- "use client"
- Fixed left sidebar (240px wide)
- Navigation links: Dashboard, Services, Products, News, Team,
  Contact Messages, Remote Resources, Pages
- Active link highlighted
- Admin email shown at bottom of sidebar
...
Write complete file: /app/admin/dashboard/page.jsx
- Server component
- Fetch counts: services, products, news, team, unread contacts, remote resources
- Display stat cards in a 3-column grid
- Each card: icon, count, label, link to that section
- Show unread contact messages count with a red badge if > 0
​
---
​
STEP 11 — ALL ADMIN CRUD PAGES
Write each completely. Every admin page must have:
- "use client"
- useEffect to fetch data on mount
- Loading state while fetching
- A table listing all records
- "Add New" button that shows a form below the table (not a modal)
- Form with all relevant fields using controlled inputs (useState)
- Slug field: auto-generated from title using:
  value.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '')
- Edit button: populates the form with existing record data
- Delete button: window.confirm() → DELETE API call → refresh list
- Save button: POST (new) or PUT (edit) → refresh list → reset form
- Error handling with user-visible error messages
- isActive toggle checkbox in every form
​
Write these files:
​
/app/admin/services/page.jsx:
  Table columns: Title, Slug, Icon, Active, Actions
  Form fields: title (auto-generates slug), iconName (text input),
    description (textarea), longDescription (textarea),
    details (comma-separated → array), features (comma-separated → array),
    technologies (comma-separated → array), isActive (checkbox)
​
/app/admin/products/page.jsx:
  Table columns: Title, Slug, Platforms, Color, Active, Actions
  Form fields: title (auto-generates slug), description (textarea),
    color (color picker input type="color"), longDescription (textarea),
    features (comma-separated → array), platforms (comma-separated → array),
    image (text input for URL), isActive (checkbox)
​
/app/admin/news/page.jsx:
  Table columns: Title, Category, Date, Active, Actions
  Form fields: title (auto-generates slug), description (textarea),
    date (text input e.g. "Dec 2024"), category (text input),
    content (textarea), coverImage (URL), author (text), isActive (checkbox)
​
/app/admin/team/page.jsx:
  Table columns: Name, Role, Order, Active, Actions
  Form fields: name, role, bio (textarea), image (URL),
    linkedin (URL), order (number), isActive (checkbox)
​
/app/admin/contact/page.jsx:
  Table columns: Name, Email, Subject, Message (first 60 chars), Read, Date, Actions
  NO add/edit form — contact messages come from public form only
  "Mark as Read" / "Mark Unread" toggle button per row
  Delete button per row
  Unread rows highlighted with light yellow background
​
/app/admin/remote-resources/page.jsx:
  Table columns: Title, Experience, Active, Actions
  Form fields: title, description (textarea), skills (comma-separated → array),
    experience (text), image (URL), isActive (checkbox)
​
/app/admin/pages/page.jsx:
  List these page slugs as clickable rows:
    home, about, services, team, products, news, contact, remote-resources
  When a row is clicked, show edit form below for that page
  Form fields: title, heroTitle, heroSubtitle,
    sections (dynamic list — each section has heading + body textarea)
    "Add Section" button appends new empty section
    "Remove" button per section removes it
  Save via PUT /api/pages/[slug]
  Create page in DB if it doesn't exist yet (PUT handler should upsert)
​
---
​
STEP 12 — FRONTEND PAGE UPDATES
Show me how to update my existing frontend pages to fetch from API instead
of importing static data. Write these 4 complete example files:
​
/app/services/page.jsx (Server Component):
- Remove import of servicesData.ts
- fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/services?active=true`,
    { next: { revalidate: 60 } })
- Render services grid exactly as before but with DB data
​
/app/services/[slug]/page.jsx (Server Component):
- fetch all services, find by slug param
- generateStaticParams() using all service slugs from API
- Show full service detail page
​
/app/products/page.jsx (Server Component):
- fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/products?active=true`,
    { next: { revalidate: 60 } })
​
/app/news/page.jsx (Server Component):
- fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/news?active=true`,
    { next: { revalidate: 60 } })
​
---
​
STEP 13 — CONTACT FORM API INTEGRATION
Write the public contact form submission. The contact page already exists.
Show how to submit to POST /api/contact from a client component:
​
/app/contact/ContactForm.jsx:
- "use client"
- Fields: name, email, phone, subject, message
- Submit to POST /api/contact (NO auth required)
...
---
​
STEP 14 — PACKAGES TO INSTALL
Write the exact npm install command with every package needed.
​
---
​
STEP 15 — HOW TO RUN SEED
Write step-by-step instructions:
1. Set up .env.local
2. Run npm install
3. Run: node scripts/seed.js
4. Verify in MongoDB Atlas that all collections are populated
5. Start dev server: npm run dev
6. Go to /admin/login and log in
7. Verify admin dashboard shows correct counts
​
---
​
IMPLEMENTATION RULES (follow strictly):
1. Write EVERY file completely — no truncation, no "..." shortcuts
2. Never write "// TODO" or "// implement this" — implement it NOW
3. Never say "similar to above" — write the full code every time
4. Each file must start with its full path as a comment on line 1
5. After completing all steps, list every file created
6. If a file is long, continue writing — do not stop mid-file
7. All admin forms must use controlled inputs with useState
8. All API calls in admin must show loading and error states
9. Every DELETE must use window.confirm() first
10. The seed file must use the EXACT data provided above — do not change any slugs, titles, or content
11. All GET routes must work without authentication (public)
12. POST /api/contact must work without authentication (public form)
13. Use upsert: true in the Pages PUT handler so pages are created if not found
​
START NOW with STEP 1. Do not write any introduction.
Begin immediately with the folder structure.